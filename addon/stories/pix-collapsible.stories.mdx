import { Meta, Story, Canvas, ArgsTable } from '@storybook/addon-docs/blocks';
import centered from '@storybook/addon-centered/ember';

import * as stories from './pix-collapsible.stories.js';

<Meta
  title='Others/Collapsible'
  component='PixCollapsible'
  argTypes={stories.argTypes}
/>

# PixCollapsible

Un `PixCollapsible` est un élément comprenant un libellé et un contenu.
Par défaut le contenu est masqué et cliquer sur le libellé permet de montrer le contenu du `PixCollapsible`.

> Il est possible de cumuler les `PixCollapsible` de sorte à former un accordéon, il suffit de les mettres dans une même div parente.

> Il est possible d'afficher une icone "plus" à la fin du composant en passant le param `withIcon` à `true`.

<Canvas>
  <Story name='PixCollapsible' story={stories.collapsible} height="120px" />
</Canvas>

<Canvas>
  <Story name='MultiplePixCollapsible' story={stories.multipleCollapsible} height="260px" />
</Canvas>

## Usage

```html
<PixCollapsible @title="Titre du contenu à dérouler en cliquant">
  <p>Contenu du PixCollapsible</p>
</PixCollapsible>
```

## Arguments

<ArgsTable story="PixCollapsible" />
