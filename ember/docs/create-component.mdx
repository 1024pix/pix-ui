...
Voici par exemple la story du composant PixNotificationAlert :

```javascript
// pix-notification-alert.stories.js
...
export const message = (args) => {
  return {
    template: hbs`
      <PixNotificationAlert @type={{type}} @withIcon={{withIcon}}>
...
`````markdown
{/_ pix-notification-alert.stories.mdx _/}
...
{/_ Affiche la prévisualisation du composant telle que définie dans pix-notification-alert.stories.js _/}
...
````html
<PixNotificationAlert @type="info" @withIcon="true"> Ceci est un message à caractère informatif. </PixNotificationAlert>
\``` ## Arguments {/* Affiche le tableau des arguments tels que définis dans pix-notification-alert.stories.js
*/}
